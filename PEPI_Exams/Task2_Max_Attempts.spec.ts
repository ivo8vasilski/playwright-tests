...
test('Task 3: Create Test with 1 min timeout, link-only and verify no answer on timeout', async ({ page, request }) => {
    ...
    // ⏳ УВЕЛИЧЕН ТАЙМАУТ: Тестът трябва да чака над 1 минута физическо време!
    ...
    const userEmail = `thomas.fan.task3.${Date.now()}@example.com`;
    const testTitle = `Auto Test Task 3 - ${Date.now()}`;
    let   accessToken: string | null = '';
    let   shareLink = ''; // Тук ще запазим генерирания линк
    ...
    await test.step('Step 9: Open Results from Dashboard', async () => {
        ...
        // Намираме теста в списъка
        const testCard = page.locator('article, li, [class*="card"], [class*="item"]')
            .filter({ hasText: testTitle })
            .first();
        
        // Търсим бутон/линк View Results
        const resultsBtn = testCard.getByRole('link', { name: /View Results|Results/i })
            .or(testCard.getByRole('button', { name: /View Results|Results/i }))
            .or(page.getByText('View Results').first());
    ...
    await test.step('Step 10: Verify NO answers in Analytics', async () => {
        // Опитваме да кликнем на таб Analytics (ако има такъв)
        const analyticsTab = page.getByRole('tab', { name: /Analytics/i }).or(page.getByText('Analytics', { exact: true }));
        if (await analyticsTab.isVisible({ timeout: 3_000 }).catch(() => false)) {
            await analyticsTab.click();
            await page.waitForLoadState('networkidle');
        }

        // Проверяваме за текст, който индикира липса на резултати ("0", "No submissions" и т.н.)
        const noDataText = page.getByText(/0 submissions|No answers yet|0 responses|No data/i)
            .or(page.locator('text="0"').first()); 
            
        await expect(noDataText).toBeVisible({ timeout: 10_000 });
    });
